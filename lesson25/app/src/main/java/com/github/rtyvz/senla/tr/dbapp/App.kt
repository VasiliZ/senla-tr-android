...
class App : Application() {
    ...
    companion object {
        ...
        private const val POPULATE_APP_DB_YET = "IS_FIRST_TIME_LAUNCH_APP"
    }
    ...
    override fun onCreate() {
        ...
        INSTANCE = this
        val prefs = getSharedPreferences(APP_PREFERENCE, Context.MODE_PRIVATE)
        if (prefs.getBoolean(POPULATE_APP_DB_YET, true)) {
            populateDB(DbProvider.provideDb())
            prefs.edit().putBoolean(POPULATE_APP_DB_YET, false).apply()
        }
    }

    private fun populateDB(db: AppDb) {
        db.populateTable(
            listOf(
